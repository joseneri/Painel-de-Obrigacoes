...
3. Consulte `tmp/` para pesquisas, achados, planos, riscos e decisoes.
4. Verifique `git status --short` para nao sobrescrever trabalho existente.
5. Classifique a tarefa como trivial, media ou complexa.
...
- Se o usuario pedir commit, antes atualize `docs/ia-showcase-guide.md` com
  decisoes tecnicas, uso de IA, validacoes e pontos de apresentacao.
...
- Endpoints chamam use cases; zero logica de negocio no endpoint.
...
- Arquitetura: a mudanca respeita `docs/architecture.md`?
...
- O proximo agente deve ler `AGENTS.md`, este arquivo, `docs/architecture.md`,
  o handoff e `git status --short`.
